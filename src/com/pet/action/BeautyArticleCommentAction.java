...
public class BeautyArticleCommentAction {
	...
	public List<Comment> getArticleContent(String questionId) {
		...
		try {
			con = DbUtil.getCon();
			ResultSet rs = beautyArticleCommentDao.getArticleContent(con, questionId);
	...
	public List<Comment> getArticleContent(String questionId,int pageNo) {
		...
		try {
			con = DbUtil.getCon();
			ResultSet rs = beautyArticleCommentDao.getArticleContent(con,questionId,pageNo);
			List<Comment> contentList = new ArrayList();
			while(rs.next()) {
				Comment content = new Comment();
				content.setAnswerName(rs.getString("answerName"));
	...
	public  void addComment(Comment comment)
	{
		...
		try {
			con = DbUtil.getCon();
			beautyArticleCommentDao.addArticleContent(con, comment);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
				DbUtil.close(con);
