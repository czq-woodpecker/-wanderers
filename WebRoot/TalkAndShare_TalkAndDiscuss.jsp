...
<html>
  ...
  <body>
  	...
  	<%
  	  							//BeautyArticleListAction dao = new BeautyArticleListAction();
				     	...
				     	<c:forEach var="articleList" items="${list }" varStatus="st" >
	                    	<article class="format-standard type-post hentry clearfix">
	                         	<header class="clearfix">
	                            	<h3 class="post-title">
	                                	<a href="TalkAndShare_TalkAndDiscuss_comment.jsp?id=${articleList.id }"><c:out value="${articleList.title }"></c:out></a>
	                                </h3>
	                                <div class="post-meta clearfix">
	                                	<span class="date">${articleList.time }</span>
	                                	<span class="category"><c:out value="${articleList.publisher }"></c:out></span>
	                                    <span class="comments"><a style="color:gray" href="TalkAndShare_TalkAndDiscuss_comment.jsp?id=${articleList.id }"><c:out value="${articleList.comments }"></c:out>&nbsp;Comments</a></span>
	                                	<span class="like-count">66</span>
	                                 </div>
	                               <!-- end of post meta -->
	                            </header>
	                            <p><c:out value="${articleList.summary }"></c:out>......<a class="readmore-link" href="TalkAndShare_TalkAndDiscuss_comment.jsp?id=${articleList.id }">Read more</a></p>
	                         </article>
                        </c:forEach>   		
                        <div id="pagination">
                          	 	<a href="TalkAndShare_TalkAndDiscuss.jsp" class="btn">首页</a>
                                <a href="TalkAndShare_TalkAndDiscuss.jsp?pageNo=1" class="btn">1</a>
                      		 	<a href="TalkAndShare_TalkAndDiscuss.jsp?pageNo=2" class="btn">2</a>
                                <a href="TalkAndShare_TalkAndDiscuss.jsp?pageNo=3" class="btn">3</a>
                                <a href="TalkAndShare_TalkAndDiscuss.jsp?pageNo=<%=pageNo-1 %> "class="btn">上一页 »</a>
                                <a href="TalkAndShare_TalkAndDiscuss.jsp?pageNo=<%=pageNo+1 %>" class="btn">下一页»</a>
                                <h4>共${total}页</h4>
                           </div>
					</div>
				...
				<aside class="span4 page-sidebar">
					<section class="wiget">
						...
						<div class="support-widget">
							...
							<button class="btn btn-link "><a href="postArticles.jsp">发布新问题求助</a></button>
						</div>
						</c:if>
					</section>
					<section class="wiget">
